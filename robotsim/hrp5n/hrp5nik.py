...
def jacobian(hrp5nrobot, armid="rgt"):
    ...
    counter = 0
    for i in hrp5nrobot.sixjoints:
        a = np.dot(armlj[i]["rotmat"], armlj[i]["rotax"])
        armjac[:, counter] = np.append(np.cross(a, armlj[hrp5nrobot.sixjoints[-1]]["linkpos"]-armlj[i]["linkpos"]), a)
        counter += 1
...
def tcperror(hrp5nrobot, tgtpos, tgtrot, armid="rgt"):
    ...
    deltapos = tgtpos - armlj[hrp5nrobot.sixjoints[-1]]["linkend"]
    deltarot = np.dot(tgtrot, armlj[hrp5nrobot.sixjoints[-1]]["rotmat"].transpose())
...
def numik(hrp5nrobot, tgtpos, tgtrot, armid="rgt"):
    ...
    # stablizer
    steplength = 30
    armjntssave = hrp5nrobot.getarmjnts6(armid)
    ...
    for i in range(500):
        ...
        if abs(np.linalg.det(armjac))>1e-6:
            ...
            hrp5nrobot.movearmfk6(armjntssave, armid)
            return None
        if np.linalg.norm(err)<1e-4:
            if hrp5nrobot.chkrng6(armjntsiter, armid):
                armjntsreturn = hrp5nrobot.getarmjnts6(armid)
                hrp5nrobot.movearmfk6(armjntssave, armid)
            ...
            if hrp5nrobot.chkrng6(armjntsiter, armid) or i < 50:
                # print armjntsiter
                hrp5nrobot.movearmfk6(armjntsiter, armid)
                # import hrp5plot
...
def numikr(hrp5nrobot, tgtpos, tgtrot, armid="rgt"):
    ...
    anglewai, angleshi = hrp5nrobot.getjntwaistshoulder(armid)
    anglewa, anglesh = eubik(tgtpos, armid)
    hrp5nrobot.movewaistshoulder(anglewa, anglesh, armid)
    armjnts6 = numik(hrp5nrobot, tgtpos, tgtrot, armid)
    hrp5nrobot.movewaistshoulder(anglewai, angleshi, armid)
    if armjnts6 is None:
        # try waist if shoulder doesnt work
        anglewa2, anglesh2 = eubik2(tgtpos, armid)
        hrp5nrobot.movewaistshoulder(anglewa2, anglesh2, armid)
        armjnts62 = numik(hrp5nrobot, tgtpos, tgtrot, armid)
        hrp5nrobot.movewaistshoulder(anglewai, angleshi, armid)
